...
function SummaryCard({ title, value, change, icon: Icon, color, trend }: SummaryCardProps) {
  return (
    <div className="bg-white/40 backdrop-blur-xl rounded-2xl p-6 border border-white/20 shadow-lg flex items-center justify-between hover:bg-white/50 transition-all">
      <div>
    ...
    <div className="space-y-8">
      ...
      <div className="bg-white/40 backdrop-blur-xl rounded-2xl border border-white/20 shadow-lg p-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3">
          {[
            { to: '/students', icon: UserPlus, color: 'bg-blue-100 text-blue-600', label: 'Add Student' },
            { to: '/staff/add', icon: GraduationCap, color: 'bg-purple-100 text-purple-600', label: 'Add Teacher' },
            { to: '/attendance', icon: Clock, color: 'bg-emerald-100 text-emerald-600', label: 'Mark Attendance' },
            { to: '/events', icon: CalendarPlus, color: 'bg-orange-100 text-orange-600', label: 'Add Event' },
            { to: '/exams', icon: FileText, color: 'bg-red-100 text-red-600', label: 'Create Exam' },
            { to: '/communication', icon: MessageSquare, color: 'bg-indigo-100 text-indigo-600', label: 'Send Message' },
          ].map((action) => (
            <Link
              key={action.label}
              to={action.to}
          ...
          <div className="h-72 w-full">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={incomeData} margin={{ top: 10, right: 10, left: 0, bottom: 0 }}>
                ...
                <Tooltip contentStyle={{ backgroundColor: 'rgba(255,255,255,0.9)', borderRadius: '12px', border: 'none', boxShadow: '0 4px 6px -1px rgba(0,0,0,0.1)' }} />
          ...
          <div className="flex justify-center gap-6 mt-4">
            {genderData.map((item) => (
              <div key={item.name} className="flex items-center gap-2">
                ...
                <span className="text-sm font-medium text-gray-600">{item.name} ({Math.round((item.value / 1020) * 100)}%)</span>
              </div>
            ))}
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="space-y-6">
            {recentActivities.map((activity) => (
              <div key={activity.id} className="flex gap-4 group">
                <div className={`w-10 h-10 rounded-full shrink-0 flex items-center justify-center ${activity.color}`}>
                ...
                <div className="flex-1 pb-6 border-b border-gray-100/50 group-last:border-none group-last:pb-0">
                  <div className="flex justify-between items-start">
                    ...
                    <span className="text-xs text-gray-400 font-medium whitespace-nowrap ml-4">{activity.time}</span>
                  </div>
                </div>
              </div>
            ))}
        ...
        <div className="bg-gradient-to-br from-indigo-600 to-purple-700 rounded-2xl shadow-xl p-6 text-white relative overflow-hidden">
          ...
          <button className="w-full mt-6 py-2.5 bg-white text-indigo-700 font-bold text-sm rounded-xl shadow-lg hover:bg-indigo-50 transition-colors relative z-10">
            View Full Calendar
          </button>
        </div>
      </div>
